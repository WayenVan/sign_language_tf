...
def main(config, checkpoint, ph14_lmdb_root, mode, device):
    ...
    for n, m in model.named_modules():
        print(n)
        if re.match(r'backbone.decoder.tf.trans_decoder.layers.[0-9]$', n):
            m.register_forward_hook(get_value)
    ...
    for j in range(6):
        axes[j].set_title('output {}'.format(j))
        # axes[j].yaxis.set_major_locator(ticker.MultipleLocator(0.1))
        axes[j].xaxis.set_major_locator(ticker.MultipleLocator(1))
        v = value[j]
        # v = torch.linalg.vector_norm(v, dim=-1)
        v = v[:, 0]
        axes[j].plot(v.numpy())
        axes[j].grid(True)
        # axes[j].set_ylim(9, 16)

    fig.savefig('resources/channel0.pdf')
    ...
    # for i in range(2):
    #     for j in range(3):
    #         axes[i][j].imshow(X=attn[i*2+j][0], vmin=0., vmax=0.5)
    #         # print(np.std(a=attn[i*2+j], axis=-1))
    #         # print(np.sum(attn[i*2+j], axis=-1))
    # plt.savefig('resources/tmp.pdf')

value = []
def get_value(m, input, output):
    value.append(output[:, 0, :].detach().cpu())
