...
class HeatMapEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(n_keypoints, neck_channels//4, kernel_size=4, stride=2, bias=False),
            nn.BatchNorm2d(neck_channels//4),
            activation(),
            nn.Conv2d(neck_channels//4, neck_channels//2, kernel_size=4, stride=2, bias=False),
            nn.BatchNorm2d(neck_channels//2),
            activation(),
            nn.Conv2d(neck_channels//2, neck_channels, kernel_size=4, stride=2, bias=False),
            nn.BatchNorm2d(neck_channels),
            activation(),
            nn.Conv2d(neck_channels, embd_dim, kernel_size=1, stride=1, bias=False),
        )
...
class VitPoseAdapterEncoder(nn.Module):
    ...
    def __init__(
        ...
        *args, **kwargs) -> None:
        ...
        self.vit: VisionTransformer = vitpose.backbone
        self.heatmap_header: HeatmapHead = vitpose.head
        self.heatmap_encoder = HeatMapEncoder(embd_dim=adapter_archs['channels'][-1], neck_channels=heatmap_neck_channels, n_keypoints=17)
        del vitpose
    ...
    def forward(self, x, t_length):
        ...
        heatmap_feats = self.heatmap_encoder(
            self.heatmap_header(intermediate_feats)
        )
    ...
    def freeze_vit(self):
        ...
        for p in self.heatmap_header.parameters():
            p.requires_grad = False
        self.vit.eval()
        self.heatmap_header.eval()
