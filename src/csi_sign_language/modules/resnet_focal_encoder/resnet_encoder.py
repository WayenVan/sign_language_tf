if __name__ == '__main__':
    import sys
    sys.path.append('src')
    from csi_sign_language.modules.resnet_focal_encoder.resnet import ResNet
    from csi_sign_language.modules.resnet_focal_encoder.components import FocalAttention
else:
...
class ResnetFocalEncoder(nn.Module):
    def __init__(
        self, 
        arch,
        ckpt, 
        ...
        *args, **kwargs) -> None:
        ...
        assert all([focal_module_cfg['stage_index'][i] < focal_module_cfg['stage_index'][i+1] for i in range(len(focal_module_cfg['stage_index']) - 1)])
    ...
    ResnetFocalEncoderOut = namedtuple('ResnetFocalEncoderOut', ['out', 't_length', 'attn_weights'])
    def forward(self, x, t_length):
        ...
        hands_tokens = self.hand_fusion(hands_tokens)
        face_tokens = self.face_fusion(face_tokens)
        hands_tokens = self.dropout_hand_token(hands_tokens)
        face_tokens = self.dropout_face_token(face_tokens)
        
        hands_tokens, face_tokens = (rearrange(a, '(b t) c -> b c t', t=T) for a in (hands_tokens, face_tokens))
        attn_weight = tuple(rearrange(a, '(b t) heads fh h w -> t b heads fh h w', t=T) for a in attn_weight)
    ...
    def pre_norm(self, x):
        x = normalize(x*255., mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375])
        return x
    ...
    def resnet_forward(self, x):
        ...
        for i, layer_name in enumerate(self.resnet.res_layers):
            ...
            if i in self.focal_attn_index:
                hand_token, face_token, attn_weight = self.focal_attentions[f'focal_attention_{i}'](x)
...
if __name__ == '__main__':
    model = ResnetFocalEncoder('resnet-18', 'resources/resnet/resnet18_8xb32_in1k_20210831-fbbb1da6.pth').to('cuda:1')
