if __name__ == "__main__":
    ...
    sys.path.append("src")
    from csi_sign_language.modules.resnet_focal_encoder.multihead_attention import (
        ScaledDotProduct,
        MultiHeadDispatch,
    )
else:
...
class FocalAttention(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.token_projection = nn.Linear(intoken_dims, embed_dims)
        self.attn = MultiHeadDispatch(embed_dims, heads, ScaledDotProduct())
    ...
    def forward(self, feats_map, hands_token, face_token):
        """
        @param feats_map B, C, H, W
        @param handstoken B, D(t-1) or D(t-1) if all batch the same
        @param face_token B, D(t-1) or D(t-1) if all batch the same
        """
        B, C, H, W = feats_map.shape
        feats_map = self.feats_projection(feats_map)
        face_token = self.token_projection(face_token)
        hands_token = self.token_projection(hands_token)

        if len(hands_token.shape) == 1:
            hands_token = rearrange(self.hands_token, "c -> () c")
        if len(face_token.shape) == 1:
