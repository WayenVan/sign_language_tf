...
@click.option(
    "--chunk_size",
    default=2,
    help="the chunk size of data submitted to each process to handle",
)
...
def main(
    ...
    shutil.copy(
        os.path.join(data_root_subset, "evaluation/phoenix2014-groundtruth-dev.stm"),
        subset_root,
    )
    shutil.copy(
        os.path.join(data_root_subset, "evaluation/phoenix2014-groundtruth-test.stm"),
        subset_root,
    )
    ...
    for mode in ("train", "dev", "test"):
        annotations, feature_dir, max_lgt_v, max_lgt_g = get_basic_info(
            data_root, mode, multisigner=multi_signer
        )
...
def process_data(
    mode, v, frame_size, annotations, feature_dir, subset, subset_root, idxes
):
    env = lmdb.open(
        os.path.join(subset_root, mode, "feature_database"), map_size=1099511627776
    )
    keys = []
    for idx in idxes:
        id, video, signer, gloss_labels = get_single_data(
            idx, annotations, v, feature_dir, frame_size
        )
        data = dict(id=id, video=video, signer=signer, gloss_labels=gloss_labels)
...
def run_mp_cmd(func, data_indexes, num_p, chunk_size):
    process_args = [
        data_indexes[i : i + chunk_size]
        for i in range(0, len(data_indexes), chunk_size)
    ]
...
def get_single_data(idx, annotations, gloss_vocab, feature_dir, frame_size=(224, 224)):
    ...
    frames: List[np.ndarray] = [
        cv2.cvtColor(cv2.resize(frame, frame_size, cv2.INTER_CUBIC), cv2.COLOR_BGR2RGB)
